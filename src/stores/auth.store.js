...
import qs from "qs";
import { useSessionStorage, useLocalStorage, useBroadcastChannel } from "@vueuse/core";
...
function attachChannel() {
  return useBroadcastChannel({ name: 'vueuse-demo-channel' })  
}
...
export const useAuthStore = () => {
  const innerStore = defineStore("auth", {
    ...
    actions: {
      fillCodeChallenge() {
        challenge_from_verifier(this.codeVerifier).then(
          (value) => (this.codeChallenge = value)
        );
      },
      login(authCode) {
        ...
        axios
          .post(
            this.keycloakURL + "/realms/Clients/protocol/openid-connect/token",
            qs.stringify(data)
          )
          .then((response) => {
      ...
      logout() {
        ...
        axios
          .post(
            this.keycloakURL + "/realms/Clients/protocol/openid-connect/logout",
            qs.stringify(data)
          )
          .then((response) => {
            console.log("Logout");
  ...
  const chanLogout = attachChannel();
  console.log(chanLogout);
  store.fillCodeChallenge();
  store.loadToken();
  store.startWatchLogout(chanLogout);
  return store;
};
